...
var interval;
var images = {
    personaje:'http://vignette4.wikia.nocookie.net/noragami/images/c/cf/Yukine_sin_fondo..png/revision/latest?cb=20150917133821&path-prefix=es',
...
class Item{
    constructor(x,y, width, height,speed,itemType,goodBad){
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
...
function generateScore(){
    ...
    //if(!(frames%100===0) ) return;
    var height = 100; //Math.floor((Math.random() * canvas.height * .6 ) + 30 );
    var score1 = new Score(500,0,100);
    var score2 = new Score(500,190, 100);
...
function checkWin(){
    if (playerOne.x || playerTwo.x >=canvas.width-200){
        clearInterval(interval);
        interval = undefined;
  ...
  function finalWinner() {
      if(playerOne.this.score < playerTwo.this.score)
        ctx.fillText ("Winner, congratulations",300,500);
        else (playerTwo.this.score < playerOne.this.score)
